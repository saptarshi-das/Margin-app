...
export function Dashboard({ courses, isDark }: DashboardProps) {
  ...
  return (
    <div className={`rounded-2xl p-5 ${
      ...
      {/* Course Overview */}
      {courses.length > 0 && (
        <div className={`rounded-xl p-4 mb-4 backdrop-blur-md border shadow-lg ${
          isDark 
          ...
          <div className="space-y-3">
            {courses.slice(0, 3).map((course) => {
              ...
              return (
                <div key={course.id} className={`flex items-center gap-3 p-2 rounded-lg ${
                  isOverLimit ? 'bg-red-500/20' : ''
                }`}>
                  {/* Course name */}
                  ...
                  <div className="flex gap-1 flex-1">
                    {Array.from({ length: course.maxLeaves }).map((_, index) => (
                      <div
                        key={index}
                        className={`h-6 rounded-full flex-1 ${
                          index < course.leaves
                            ? 'bg-red-500'
                            : isDark 
                              ? 'bg-gray-300' 
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className={`rounded-xl p-3 backdrop-blur-md border shadow-lg ${
          isDark 
            ? 'bg-white/20 border-white/30 shadow-black/5' 
